...
export default async function getProfilePostsAction(userId: number) {
  ...
  const posts = await Model.query(
    `SELECT *,(${getUserByObjectQuery(
      ...
      "SP.userId",
    )}),'Photos',(SELECT JSON_ARRAYAGG(JSON_OBJECT('id',SPH.id,'height',SPH.height,'width',SPH.width,'filename',SPH.filename)) FROM Photos AS SPH WHERE SPH.postId=SP.id)) FROM Posts SP WHERE SP.id=P.sharedId) AS SharedPost FROM Posts AS P WHERE P.userId=${userId} ORDER BY P.id DESC LIMIT 100`,
  );
  return posts;
}
