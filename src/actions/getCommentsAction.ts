...
export default async function getCommentsAction({
  ...
  const comments = await Model.query(
    `SELECT *,${
      ...
      user?.id
    } AND MR.commentId=CMT.id) AS myReactionType,(SELECT COUNT(*) FROM Reactions R WHERE R.commentId=CMT.id) AS totalReactions,(SELECT JSON_ARRAYAGG(JSON_OBJECT('id',RPC.id,'uuId',RPC.uuId,'text',RPC.text,'postOwnerId',${postOwnerId},'currentUserId',${
      user?.id
